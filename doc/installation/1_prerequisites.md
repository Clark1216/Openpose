...
3. Nvidia GPU version prerequisites:
    ...
    2. Upgrade your Nvidia drivers to the latest version.
        ...
        (**DUDU Note**: If cannot use runfile to install driver due to legacy issues, you can use additional driver GUI to upgrade, as long as the version is above 515.00.)
    3. **CUDA**: You can simply run `sudo bash ./scripts/ubuntu/install_cuda.sh` if you are not too familiar with CUDA. If you are, then you could also do one of the following instead:
        - Ubuntu 20 ([**CUDA 11.7.1**](https://developer.nvidia.com/cuda-11-7-1-download-archive)): Download CUDA 11.7.1 from their [official website](https://developer.nvidia.com/cuda-11-7-1-download-archive). Most Ubuntu computers use the `Architecture` named `x86_64`, and we personally recommend the `Installer Type` named `runfile (local)`. Then, follow the Nvidia website installation instructions. When installing, make sure to enable the symbolic link in `usr/local/cuda` to minimize potential future errors. (**DUDU Note**: `sudo unlink /usr/local/cuda` and `sudo ln -s /usr/local/cuda-11.7 /usr/local/cuda` ) If the (Nvidia) drivers were installed manually, untick the "install driver" option. (**DUDU Note**: Use `nvcc --version` to verify installtion of cuda toolkit after reboot)
